...
def inference_person(image):
    with tf.variable_scope('PersonNet'):
        ...
        print('pool1_stage1:' + str(Mconv7_stage4.shape))
    return Mconv7_stage4, Mconv7_stage3, Mconv7_stage2, conv6_2_CPM
...
def inference_pose(image, center_map):
    ...
    return Mconv5_stage6, Mconv5_stage5, Mconv5_stage4, Mconv5_stage3, Mconv5_stage2, conv7_stage1

def inference_pose_E4(image, center_map):
    # corresponds to pose_deploy_centerMap.prototxt
    with tf.variable_scope('PoseNet'):
        pool_center_lower = layers.avg_pool2d(center_map, 9, 8, padding='VALID')


        # conv1_stage1_encoder = layers.conv2d(image[:,:,:,3:4], 128, 9, 1, activation_fn=None, scope='conv1_stage2_encoder')
        # conv1_stage1_encoder = tf.nn.relu(conv1_stage1_encoder)
        # pool1_stage1_encoder = layers.max_pool2d(conv1_stage1_encoder, 3, 2)
        # conv2_stage1_encoder = layers.conv2d(pool1_stage1_encoder, 128, 9, 1, activation_fn=None, scope='conv2_stage2_encoder')
        # conv2_stage1_encoder = tf.nn.relu(conv2_stage1_encoder)
        # pool2_stage1_encoder = layers.max_pool2d(conv2_stage1_encoder, 3, 2)
        # conv3_stage1_encoder = layers.conv2d(pool2_stage1_encoder, 128, 9, 1, activation_fn=None, scope='conv3_stage2_encoder')
        # conv3_stage1_encoder = tf.nn.relu(conv3_stage1_encoder)
        # pool3_stage1_encoder = layers.max_pool2d(conv3_stage1_encoder, 3, 2)
        # conv4_stage1_encoder = layers.conv2d(pool3_stage1_encoder, 32, 5, 1, activation_fn=None, scope='conv4_stage2_encoder')
        # conv4_stage1_encoder = tf.nn.relu(conv4_stage1_encoder)

        # ---------------------------------------------------------
        # original first layer: conv1_stage1 = layers.conv2d(image, 128, 9, 1, activation_fn=None, scope='conv1_stage1')
        conv1_stage1 = layers.conv2d(image, 128, 9, 1, activation_fn=None, scope='conv1_stage1_4c')
        conv1_stage1 = tf.nn.relu(conv1_stage1)
        pool1_stage1 = layers.max_pool2d(conv1_stage1, 3, 2)
        conv2_stage1 = layers.conv2d(pool1_stage1, 128, 9, 1, activation_fn=None, scope='conv2_stage1')
        conv2_stage1 = tf.nn.relu(conv2_stage1)
        ...
        conv6_stage1 = tf.nn.relu(conv6_stage1)
        conv7_stage1 = layers.conv2d(conv6_stage1, 15, 1, 1, activation_fn=None, scope='conv7_stage1')
        conv1_stage2 = layers.conv2d(image, 128, 9, 1, activation_fn=None, scope='conv1_stage2_4c')
        conv1_stage2 = tf.nn.relu(conv1_stage2)
        pool1_stage2 = layers.max_pool2d(conv1_stage2, 3, 2)
        conv2_stage2 = layers.conv2d(pool1_stage2, 128, 9, 1, activation_fn=None, scope='conv2_stage2')
        conv2_stage2 = tf.nn.relu(conv2_stage2)
